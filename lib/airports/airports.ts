export interface Airport { code:string; city:string; name:string; country:string; flag:string; countryCode?:string; ident?:string; }

export const airports:Airport[]=[
  {
    "code": "CDG",
    "city": "Paris",
    "name": "Charles de Gaulle",
    "country": "France",
    "flag": "🇫🇷"
  },
  {
    "code": "ORY",
    "city": "Paris",
    "name": "Orly",
    "country": "France",
    "flag": "🇫🇷"
  },
  {
    "code": "NCE",
    "city": "Nice",
    "name": "Côte d'Azur",
    "country": "France",
    "flag": "🇫🇷"
  },
  {
    "code": "LYS",
    "city": "Lyon",
    "name": "Saint-Exupéry",
    "country": "France",
    "flag": "🇫🇷"
  },
  {
    "code": "MRS",
    "city": "Marseille",
    "name": "Provence",
    "country": "France",
    "flag": "🇫🇷"
  },
  {
    "code": "TLS",
    "city": "Toulouse",
    "name": "Blagnac",
    "country": "France",
    "flag": "🇫🇷"
  },
  {
    "code": "BOD",
    "city": "Bordeaux",
    "name": "Mérignac",
    "country": "France",
    "flag": "🇫🇷"
  },
  {
    "code": "NTE",
    "city": "Nantes",
    "name": "Atlantique",
    "country": "France",
    "flag": "🇫🇷"
  },
  {
    "code": "SXB",
    "city": "Strasbourg",
    "name": "Entzheim",
    "country": "France",
    "flag": "🇫🇷"
  },
  {
    "code": "BES",
    "city": "Brest",
    "name": "Guipavas",
    "country": "France",
    "flag": "🇫🇷"
  },
  {
    "code": "RNS",
    "city": "Rennes",
    "name": "Saint-Jacques",
    "country": "France",
    "flag": "🇫🇷"
  },
  {
    "code": "MLH",
    "city": "Mulhouse",
    "name": "EuroAirport",
    "country": "France",
    "flag": "🇫🇷"
  },
  {
    "code": "MPL",
    "city": "Montpellier",
    "name": "Fréjorgues",
    "country": "France",
    "flag": "🇫🇷"
  },
  {
    "code": "GVA",
    "city": "Genève",
    "name": "Genève",
    "country": "Suisse",
    "flag": "🇨🇭"
  },
  {
    "code": "LHR",
    "city": "Londres",
    "name": "Heathrow",
    "country": "Royaume-Uni",
    "flag": "🇬🇧"
  },
  {
    "code": "LGW",
    "city": "Londres",
    "name": "Gatwick",
    "country": "Royaume-Uni",
    "flag": "🇬🇧"
  },
  {
    "code": "STN",
    "city": "Londres",
    "name": "Stansted",
    "country": "Royaume-Uni",
    "flag": "🇬🇧"
  },
  {
    "code": "LTN",
    "city": "Londres",
    "name": "Luton",
    "country": "Royaume-Uni",
    "flag": "🇬🇧"
  },
  {
    "code": "MAN",
    "city": "Manchester",
    "name": "Manchester",
    "country": "Royaume-Uni",
    "flag": "🇬🇧"
  },
  {
    "code": "EDI",
    "city": "Édimbourg",
    "name": "Edinburgh",
    "country": "Royaume-Uni",
    "flag": "🇬🇧"
  },
  {
    "code": "BHX",
    "city": "Birmingham",
    "name": "Birmingham",
    "country": "Royaume-Uni",
    "flag": "🇬🇧"
  },
  {
    "code": "MAD",
    "city": "Madrid",
    "name": "Barajas",
    "country": "Espagne",
    "flag": "🇪🇸"
  },
  {
    "code": "BCN",
    "city": "Barcelone",
    "name": "El Prat",
    "country": "Espagne",
    "flag": "🇪🇸"
  },
  {
    "code": "AGP",
    "city": "Malaga",
    "name": "Costa del Sol",
    "country": "Espagne",
    "flag": "🇪🇸"
  },
  {
    "code": "PMI",
    "city": "Palma",
    "name": "Son Sant Joan",
    "country": "Espagne",
    "flag": "🇪🇸"
  },
  {
    "code": "ALC",
    "city": "Alicante",
    "name": "El Altet",
    "country": "Espagne",
    "flag": "🇪🇸"
  },
  {
    "code": "VLC",
    "city": "Valence",
    "name": "Valencia",
    "country": "Espagne",
    "flag": "🇪🇸"
  },
  {
    "code": "SVQ",
    "city": "Séville",
    "name": "San Pablo",
    "country": "Espagne",
    "flag": "🇪🇸"
  },
  {
    "code": "IBZ",
    "city": "Ibiza",
    "name": "Ibiza",
    "country": "Espagne",
    "flag": "🇪🇸"
  },
  {
    "code": "TFS",
    "city": "Tenerife",
    "name": "Sur",
    "country": "Espagne",
    "flag": "🇪🇸"
  },
  {
    "code": "LPA",
    "city": "Gran Canaria",
    "name": "Las Palmas",
    "country": "Espagne",
    "flag": "🇪🇸"
  },
  {
    "code": "FCO",
    "city": "Rome",
    "name": "Fiumicino",
    "country": "Italie",
    "flag": "🇮🇹"
  },
  {
    "code": "MXP",
    "city": "Milan",
    "name": "Malpensa",
    "country": "Italie",
    "flag": "🇮🇹"
  },
  {
    "code": "BGY",
    "city": "Bergame",
    "name": "Orio al Serio",
    "country": "Italie",
    "flag": "🇮🇹"
  },
  {
    "code": "VCE",
    "city": "Venise",
    "name": "Marco Polo",
    "country": "Italie",
    "flag": "🇮🇹"
  },
  {
    "code": "NAP",
    "city": "Naples",
    "name": "Capodichino",
    "country": "Italie",
    "flag": "🇮🇹"
  },
  {
    "code": "CTA",
    "city": "Catane",
    "name": "Fontanarossa",
    "country": "Italie",
    "flag": "🇮🇹"
  },
  {
    "code": "FLR",
    "city": "Florence",
    "name": "Peretola",
    "country": "Italie",
    "flag": "🇮🇹"
  },
  {
    "code": "BLQ",
    "city": "Bologne",
    "name": "Guglielmo Marconi",
    "country": "Italie",
    "flag": "🇮🇹"
  },
  {
    "code": "FRA",
    "city": "Francfort",
    "name": "Frankfurt",
    "country": "Allemagne",
    "flag": "🇩🇪"
  },
  {
    "code": "MUC",
    "city": "Munich",
    "name": "Munich",
    "country": "Allemagne",
    "flag": "🇩🇪"
  },
  {
    "code": "BER",
    "city": "Berlin",
    "name": "Brandenburg",
    "country": "Allemagne",
    "flag": "🇩🇪"
  },
  {
    "code": "HAM",
    "city": "Hambourg",
    "name": "Hamburg",
    "country": "Allemagne",
    "flag": "🇩🇪"
  },
  {
    "code": "DUS",
    "city": "Düsseldorf",
    "name": "Düsseldorf",
    "country": "Allemagne",
    "flag": "🇩🇪"
  },
  {
    "code": "CGN",
    "city": "Cologne",
    "name": "Cologne Bonn",
    "country": "Allemagne",
    "flag": "🇩🇪"
  },
  {
    "code": "STR",
    "city": "Stuttgart",
    "name": "Stuttgart",
    "country": "Allemagne",
    "flag": "🇩🇪"
  },
  {
    "code": "AMS",
    "city": "Amsterdam",
    "name": "Schiphol",
    "country": "Pays-Bas",
    "flag": "🇳🇱"
  },
  {
    "code": "BRU",
    "city": "Bruxelles",
    "name": "Brussels",
    "country": "Belgique",
    "flag": "🇧🇪"
  },
  {
    "code": "CRL",
    "city": "Charleroi",
    "name": "Charleroi",
    "country": "Belgique",
    "flag": "🇧🇪"
  },
  {
    "code": "LIS",
    "city": "Lisbonne",
    "name": "Humberto Delgado",
    "country": "Portugal",
    "flag": "🇵🇹"
  },
  {
    "code": "OPO",
    "city": "Porto",
    "name": "Francisco Sá Carneiro",
    "country": "Portugal",
    "flag": "🇵🇹"
  },
  {
    "code": "FAO",
    "city": "Faro",
    "name": "Faro",
    "country": "Portugal",
    "flag": "🇵🇹"
  },
  {
    "code": "ATH",
    "city": "Athènes",
    "name": "Eleftherios Venizelos",
    "country": "Grèce",
    "flag": "🇬🇷"
  },
  {
    "code": "SKG",
    "city": "Thessalonique",
    "name": "Makedonia",
    "country": "Grèce",
    "flag": "🇬🇷"
  },
  {
    "code": "HER",
    "city": "Héraklion",
    "name": "Nikos Kazantzakis",
    "country": "Grèce",
    "flag": "🇬🇷"
  },
  {
    "code": "RHO",
    "city": "Rhodes",
    "name": "Diagoras",
    "country": "Grèce",
    "flag": "🇬🇷"
  },
  {
    "code": "CFU",
    "city": "Corfou",
    "name": "Ioannis Kapodistrias",
    "country": "Grèce",
    "flag": "🇬🇷"
  },
  {
    "code": "JMK",
    "city": "Mykonos",
    "name": "Mykonos",
    "country": "Grèce",
    "flag": "🇬🇷"
  },
  {
    "code": "JSI",
    "city": "Skiathos",
    "name": "Skiathos",
    "country": "Grèce",
    "flag": "🇬🇷"
  },
  {
    "code": "SMI",
    "city": "Samos",
    "name": "Samos",
    "country": "Grèce",
    "flag": "🇬🇷"
  },
  {
    "code": "IST",
    "city": "Istanbul",
    "name": "Istanbul",
    "country": "Turquie",
    "flag": "🇹🇷"
  },
  {
    "code": "SAW",
    "city": "Istanbul",
    "name": "Sabiha Gökçen",
    "country": "Turquie",
    "flag": "🇹🇷"
  },
  {
    "code": "AYT",
    "city": "Antalya",
    "name": "Antalya",
    "country": "Turquie",
    "flag": "🇹🇷"
  },
  {
    "code": "ADB",
    "city": "Izmir",
    "name": "Adnan Menderes",
    "country": "Turquie",
    "flag": "🇹🇷"
  },
  {
    "code": "ESB",
    "city": "Ankara",
    "name": "Esenboğa",
    "country": "Turquie",
    "flag": "🇹🇷"
  },
  {
    "code": "DXB",
    "city": "Dubaï",
    "name": "Dubai International",
    "country": "Émirats Arabes",
    "flag": "🇦🇪"
  },
  {
    "code": "DWC",
    "city": "Dubaï",
    "name": "Al Maktoum",
    "country": "Émirats Arabes",
    "flag": "🇦🇪"
  },
  {
    "code": "AUH",
    "city": "Abu Dhabi",
    "name": "Abu Dhabi",
    "country": "Émirats Arabes",
    "flag": "🇦🇪"
  },
  {
    "code": "SHJ",
    "city": "Sharjah",
    "name": "Sharjah",
    "country": "Émirats Arabes",
    "flag": "🇦🇪"
  },
  {
    "code": "DOH",
    "city": "Doha",
    "name": "Hamad",
    "country": "Qatar",
    "flag": "🇶🇦"
  },
  {
    "code": "RUH",
    "city": "Riyad",
    "name": "King Khalid",
    "country": "Arabie Saoudite",
    "flag": "🇸🇦"
  },
  {
    "code": "JED",
    "city": "Djeddah",
    "name": "King Abdulaziz",
    "country": "Arabie Saoudite",
    "flag": "🇸🇦"
  },
  {
    "code": "KWI",
    "city": "Koweït",
    "name": "Kuwait",
    "country": "Koweït",
    "flag": "🇰🇼"
  },
  {
    "code": "BAH",
    "city": "Manama",
    "name": "Bahrain",
    "country": "Bahreïn",
    "flag": "🇧🇭"
  },
  {
    "code": "MCT",
    "city": "Mascate",
    "name": "Muscat",
    "country": "Oman",
    "flag": "🇴🇲"
  },
  {
    "code": "AMM",
    "city": "Amman",
    "name": "Queen Alia",
    "country": "Jordanie",
    "flag": "🇯🇴"
  },
  {
    "code": "BEY",
    "city": "Beyrouth",
    "name": "Rafic Hariri",
    "country": "Liban",
    "flag": "🇱🇧"
  },
  {
    "code": "TLV",
    "city": "Tel Aviv",
    "name": "Ben Gurion",
    "country": "Israël",
    "flag": "🇮🇱"
  },
  {
    "code": "KUL",
    "city": "Kuala Lumpur",
    "name": "KLIA",
    "country": "Malaisie",
    "flag": "🇲🇾"
  },
  {
    "code": "SIN",
    "city": "Singapour",
    "name": "Changi",
    "country": "Singapour",
    "flag": "🇸🇬"
  },
  {
    "code": "BKK",
    "city": "Bangkok",
    "name": "Suvarnabhumi",
    "country": "Thaïlande",
    "flag": "🇹🇭"
  },
  {
    "code": "DMK",
    "city": "Bangkok",
    "name": "Don Mueang",
    "country": "Thaïlande",
    "flag": "🇹🇭"
  },
  {
    "code": "HKT",
    "city": "Phuket",
    "name": "Phuket",
    "country": "Thaïlande",
    "flag": "🇹🇭"
  },
  {
    "code": "CNX",
    "city": "Chiang Mai",
    "name": "Chiang Mai",
    "country": "Thaïlande",
    "flag": "🇹🇭"
  },
  {
    "code": "DPS",
    "city": "Bali",
    "name": "Ngurah Rai",
    "country": "Indonésie",
    "flag": "🇮🇩"
  },
  {
    "code": "CGK",
    "city": "Jakarta",
    "name": "Soekarno-Hatta",
    "country": "Indonésie",
    "flag": "🇮🇩"
  },
  {
    "code": "MNL",
    "city": "Manille",
    "name": "Ninoy Aquino",
    "country": "Philippines",
    "flag": "🇵🇭"
  },
  {
    "code": "SGN",
    "city": "Hô Chi Minh",
    "name": "Tan Son Nhat",
    "country": "Vietnam",
    "flag": "🇻🇳"
  },
  {
    "code": "HAN",
    "city": "Hanoi",
    "name": "Noi Bai",
    "country": "Vietnam",
    "flag": "🇻🇳"
  },
  {
    "code": "DAD",
    "city": "Da Nang",
    "name": "Da Nang",
    "country": "Vietnam",
    "flag": "🇻🇳"
  },
  {
    "code": "PNH",
    "city": "Phnom Penh",
    "name": "Pochentong",
    "country": "Cambodge",
    "flag": "🇰🇭"
  },
  {
    "code": "REP",
    "city": "Siem Reap",
    "name": "Siem Reap",
    "country": "Cambodge",
    "flag": "🇰🇭"
  },
  {
    "code": "VTE",
    "city": "Vientiane",
    "name": "Wattay",
    "country": "Laos",
    "flag": "🇱🇦"
  },
  {
    "code": "RGN",
    "city": "Yangon",
    "name": "Yangon",
    "country": "Myanmar",
    "flag": "🇲🇲"
  },
  {
    "code": "LGK",
    "city": "Langkawi",
    "name": "Langkawi",
    "country": "Malaisie",
    "flag": "🇲🇾"
  },
  {
    "code": "BKI",
    "city": "Kota Kinabalu",
    "name": "Kota Kinabalu",
    "country": "Malaisie",
    "flag": "🇲🇾"
  },
  {
    "code": "JHB",
    "city": "Johor Bahru",
    "name": "Senai",
    "country": "Malaisie",
    "flag": "🇲🇾"
  },
  {
    "code": "DEL",
    "city": "New Delhi",
    "name": "Indira Gandhi",
    "country": "Inde",
    "flag": "🇮🇳"
  },
  {
    "code": "BOM",
    "city": "Mumbai",
    "name": "Chhatrapati Shivaji",
    "country": "Inde",
    "flag": "🇮🇳"
  },
  {
    "code": "BLR",
    "city": "Bangalore",
    "name": "Kempegowda",
    "country": "Inde",
    "flag": "🇮🇳"
  },
  {
    "code": "MAA",
    "city": "Chennai",
    "name": "Chennai",
    "country": "Inde",
    "flag": "🇮🇳"
  },
  {
    "code": "CCU",
    "city": "Calcutta",
    "name": "Netaji Subhash Chandra",
    "country": "Inde",
    "flag": "🇮🇳"
  },
  {
    "code": "HYD",
    "city": "Hyderabad",
    "name": "Rajiv Gandhi",
    "country": "Inde",
    "flag": "🇮🇳"
  },
  {
    "code": "CMB",
    "city": "Colombo",
    "name": "Bandaranaike",
    "country": "Sri Lanka",
    "flag": "🇱🇰"
  },
  {
    "code": "DAC",
    "city": "Dhaka",
    "name": "Hazrat Shahjalal",
    "country": "Bangladesh",
    "flag": "🇧🇩"
  },
  {
    "code": "KTM",
    "city": "Katmandou",
    "name": "Tribhuvan",
    "country": "Népal",
    "flag": "🇳🇵"
  },
  {
    "code": "MLE",
    "city": "Malé",
    "name": "Velana",
    "country": "Maldives",
    "flag": "🇲🇻"
  },
  {
    "code": "PEK",
    "city": "Pékin",
    "name": "Capital",
    "country": "Chine",
    "flag": "🇨🇳"
  },
  {
    "code": "PKX",
    "city": "Pékin",
    "name": "Daxing",
    "country": "Chine",
    "flag": "🇨🇳"
  },
  {
    "code": "PVG",
    "city": "Shanghai",
    "name": "Pudong",
    "country": "Chine",
    "flag": "🇨🇳"
  },
  {
    "code": "SHA",
    "city": "Shanghai",
    "name": "Hongqiao",
    "country": "Chine",
    "flag": "🇨🇳"
  },
  {
    "code": "CAN",
    "city": "Guangzhou",
    "name": "Baiyun",
    "country": "Chine",
    "flag": "🇨🇳"
  },
  {
    "code": "HKG",
    "city": "Hong Kong",
    "name": "Hong Kong",
    "country": "Hong Kong",
    "flag": "🇭🇰"
  },
  {
    "code": "TPE",
    "city": "Taipei",
    "name": "Taoyuan",
    "country": "Taïwan",
    "flag": "🇹🇼"
  },
  {
    "code": "NRT",
    "city": "Tokyo",
    "name": "Narita",
    "country": "Japon",
    "flag": "🇯🇵"
  },
  {
    "code": "HND",
    "city": "Tokyo",
    "name": "Haneda",
    "country": "Japon",
    "flag": "🇯🇵"
  },
  {
    "code": "KIX",
    "city": "Osaka",
    "name": "Kansai",
    "country": "Japon",
    "flag": "🇯🇵"
  },
  {
    "code": "CTS",
    "city": "Sapporo",
    "name": "New Chitose",
    "country": "Japon",
    "flag": "🇯🇵"
  },
  {
    "code": "FUK",
    "city": "Fukuoka",
    "name": "Fukuoka",
    "country": "Japon",
    "flag": "🇯🇵"
  },
  {
    "code": "ICN",
    "city": "Séoul",
    "name": "Incheon",
    "country": "Corée du Sud",
    "flag": "🇰🇷"
  },
  {
    "code": "GMP",
    "city": "Séoul",
    "name": "Gimpo",
    "country": "Corée du Sud",
    "flag": "🇰🇷"
  },
  {
    "code": "CMN",
    "city": "Casablanca",
    "name": "Mohammed V",
    "country": "Maroc",
    "flag": "🇲🇦"
  },
  {
    "code": "RAK",
    "city": "Marrakech",
    "name": "Menara",
    "country": "Maroc",
    "flag": "🇲🇦"
  },
  {
    "code": "AGA",
    "city": "Agadir",
    "name": "Al Massira",
    "country": "Maroc",
    "flag": "🇲🇦"
  },
  {
    "code": "TNG",
    "city": "Tanger",
    "name": "Ibn Batouta",
    "country": "Maroc",
    "flag": "🇲🇦"
  },
  {
    "code": "FEZ",
    "city": "Fès",
    "name": "Fès-Saïss",
    "country": "Maroc",
    "flag": "🇲🇦"
  },
  {
    "code": "ALG",
    "city": "Alger",
    "name": "Houari Boumediene",
    "country": "Algérie",
    "flag": "🇩🇿"
  },
  {
    "code": "TUN",
    "city": "Tunis",
    "name": "Carthage",
    "country": "Tunisie",
    "flag": "🇹🇳"
  },
  {
    "code": "DJE",
    "city": "Djerba",
    "name": "Zarzis",
    "country": "Tunisie",
    "flag": "🇹🇳"
  },
  {
    "code": "CAI",
    "city": "Le Caire",
    "name": "Cairo",
    "country": "Égypte",
    "flag": "🇪🇬"
  },
  {
    "code": "HRG",
    "city": "Hurghada",
    "name": "Hurghada",
    "country": "Égypte",
    "flag": "🇪🇬"
  },
  {
    "code": "SSH",
    "city": "Charm el-Cheikh",
    "name": "Sharm El Sheikh",
    "country": "Égypte",
    "flag": "🇪🇬"
  },
  {
    "code": "LOS",
    "city": "Lagos",
    "name": "Murtala Muhammed",
    "country": "Nigeria",
    "flag": "🇳🇬"
  },
  {
    "code": "ABV",
    "city": "Abuja",
    "name": "Abuja",
    "country": "Nigeria",
    "flag": "🇳🇬"
  },
  {
    "code": "ACC",
    "city": "Accra",
    "name": "Kotoka",
    "country": "Ghana",
    "flag": "🇬🇭"
  },
  {
    "code": "DKR",
    "city": "Dakar",
    "name": "Blaise Diagne",
    "country": "Sénégal",
    "flag": "🇸🇳"
  },
  {
    "code": "ABJ",
    "city": "Abidjan",
    "name": "Félix Houphouët-Boigny",
    "country": "Côte d'Ivoire",
    "flag": "🇨🇮"
  },
  {
    "code": "COO",
    "city": "Cotonou",
    "name": "Cadjehoun",
    "country": "Bénin",
    "flag": "🇧🇯"
  },
  {
    "code": "OUA",
    "city": "Ouagadougou",
    "name": "Thomas Sankara",
    "country": "Burkina Faso",
    "flag": "🇧🇫"
  },
  {
    "code": "BKO",
    "city": "Bamako",
    "name": "Bamako-Sénou",
    "country": "Mali",
    "flag": "🇲🇱"
  },
  {
    "code": "NIM",
    "city": "Niamey",
    "name": "Diori Hamani",
    "country": "Niger",
    "flag": "🇳🇪"
  },
  {
    "code": "LFW",
    "city": "Lomé",
    "name": "Gnassingbé Eyadéma",
    "country": "Togo",
    "flag": "🇹🇬"
  },
  {
    "code": "ROB",
    "city": "Monrovia",
    "name": "Roberts",
    "country": "Liberia",
    "flag": "🇱🇷"
  },
  {
    "code": "NBO",
    "city": "Nairobi",
    "name": "Jomo Kenyatta",
    "country": "Kenya",
    "flag": "🇰🇪"
  },
  {
    "code": "ADD",
    "city": "Addis-Abeba",
    "name": "Bole",
    "country": "Éthiopie",
    "flag": "🇪🇹"
  },
  {
    "code": "DAR",
    "city": "Dar es Salam",
    "name": "Julius Nyerere",
    "country": "Tanzanie",
    "flag": "🇹🇿"
  },
  {
    "code": "JRO",
    "city": "Kilimanjaro",
    "name": "Kilimanjaro",
    "country": "Tanzanie",
    "flag": "🇹🇿"
  },
  {
    "code": "ZNZ",
    "city": "Zanzibar",
    "name": "Abeid Amani Karume",
    "country": "Tanzanie",
    "flag": "🇹🇿"
  },
  {
    "code": "JNB",
    "city": "Johannesburg",
    "name": "OR Tambo",
    "country": "Afrique du Sud",
    "flag": "🇿🇦"
  },
  {
    "code": "CPT",
    "city": "Le Cap",
    "name": "Cape Town",
    "country": "Afrique du Sud",
    "flag": "🇿🇦"
  },
  {
    "code": "MRU",
    "city": "Maurice",
    "name": "Sir Seewoosagur Ramgoolam",
    "country": "Île Maurice",
    "flag": "🇲🇺"
  },
  {
    "code": "RUN",
    "city": "La Réunion",
    "name": "Roland Garros",
    "country": "La Réunion",
    "flag": "🇷🇪"
  },
  {
    "code": "TNR",
    "city": "Antananarivo",
    "name": "Ivato",
    "country": "Madagascar",
    "flag": "🇲🇬"
  },
  {
    "code": "JFK",
    "city": "New York",
    "name": "John F. Kennedy",
    "country": "États-Unis",
    "flag": "🇺🇸"
  },
  {
    "code": "LGA",
    "city": "New York",
    "name": "LaGuardia",
    "country": "États-Unis",
    "flag": "🇺🇸"
  },
  {
    "code": "EWR",
    "city": "New York",
    "name": "Newark",
    "country": "États-Unis",
    "flag": "🇺🇸"
  },
  {
    "code": "LAX",
    "city": "Los Angeles",
    "name": "Los Angeles",
    "country": "États-Unis",
    "flag": "🇺🇸"
  },
  {
    "code": "ORD",
    "city": "Chicago",
    "name": "O'Hare",
    "country": "États-Unis",
    "flag": "🇺🇸"
  },
  {
    "code": "ATL",
    "city": "Atlanta",
    "name": "Hartsfield-Jackson",
    "country": "États-Unis",
    "flag": "🇺🇸"
  },
  {
    "code": "DFW",
    "city": "Dallas",
    "name": "Dallas Fort Worth",
    "country": "États-Unis",
    "flag": "🇺🇸"
  },
  {
    "code": "MIA",
    "city": "Miami",
    "name": "Miami",
    "country": "États-Unis",
    "flag": "🇺🇸"
  },
  {
    "code": "SFO",
    "city": "San Francisco",
    "name": "San Francisco",
    "country": "États-Unis",
    "flag": "🇺🇸"
  },
  {
    "code": "LAS",
    "city": "Las Vegas",
    "name": "Harry Reid",
    "country": "États-Unis",
    "flag": "🇺🇸"
  },
  {
    "code": "BOS",
    "city": "Boston",
    "name": "Logan",
    "country": "États-Unis",
    "flag": "🇺🇸"
  },
  {
    "code": "SEA",
    "city": "Seattle",
    "name": "Sea-Tac",
    "country": "États-Unis",
    "flag": "🇺🇸"
  },
  {
    "code": "DEN",
    "city": "Denver",
    "name": "Denver",
    "country": "États-Unis",
    "flag": "🇺🇸"
  },
  {
    "code": "MCO",
    "city": "Orlando",
    "name": "Orlando",
    "country": "États-Unis",
    "flag": "🇺🇸"
  },
  {
    "code": "YYZ",
    "city": "Toronto",
    "name": "Pearson",
    "country": "Canada",
    "flag": "🇨🇦"
  },
  {
    "code": "YUL",
    "city": "Montréal",
    "name": "Montréal-Trudeau",
    "country": "Canada",
    "flag": "🇨🇦"
  },
  {
    "code": "YVR",
    "city": "Vancouver",
    "name": "Vancouver",
    "country": "Canada",
    "flag": "🇨🇦"
  },
  {
    "code": "MEX",
    "city": "Mexico",
    "name": "Benito Juárez",
    "country": "Mexique",
    "flag": "🇲🇽"
  },
  {
    "code": "CUN",
    "city": "Cancún",
    "name": "Cancún",
    "country": "Mexique",
    "flag": "🇲🇽"
  },
  {
    "code": "GRU",
    "city": "São Paulo",
    "name": "Guarulhos",
    "country": "Brésil",
    "flag": "🇧🇷"
  },
  {
    "code": "GIG",
    "city": "Rio de Janeiro",
    "name": "Galeão",
    "country": "Brésil",
    "flag": "🇧🇷"
  },
  {
    "code": "EZE",
    "city": "Buenos Aires",
    "name": "Ministro Pistarini",
    "country": "Argentine",
    "flag": "🇦🇷"
  },
  {
    "code": "BOG",
    "city": "Bogotá",
    "name": "El Dorado",
    "country": "Colombie",
    "flag": "🇨🇴"
  },
  {
    "code": "LIM",
    "city": "Lima",
    "name": "Jorge Chávez",
    "country": "Pérou",
    "flag": "🇵🇪"
  },
  {
    "code": "SCL",
    "city": "Santiago",
    "name": "Arturo Merino Benítez",
    "country": "Chili",
    "flag": "🇨🇱"
  },
  {
    "code": "SYD",
    "city": "Sydney",
    "name": "Kingsford Smith",
    "country": "Australie",
    "flag": "🇦🇺"
  },
  {
    "code": "MEL",
    "city": "Melbourne",
    "name": "Melbourne",
    "country": "Australie",
    "flag": "🇦🇺"
  },
  {
    "code": "BNE",
    "city": "Brisbane",
    "name": "Brisbane",
    "country": "Australie",
    "flag": "🇦🇺"
  },
  {
    "code": "PER",
    "city": "Perth",
    "name": "Perth",
    "country": "Australie",
    "flag": "🇦🇺"
  },
  {
    "code": "AKL",
    "city": "Auckland",
    "name": "Auckland",
    "country": "Nouvelle-Zélande",
    "flag": "🇳🇿"
  },
  {
    "code": "CHC",
    "city": "Christchurch",
    "name": "Christchurch",
    "country": "Nouvelle-Zélande",
    "flag": "🇳🇿"
  },
  {
    "code": "PPT",
    "city": "Papeete",
    "name": "Faa'a",
    "country": "Polynésie française",
    "flag": "🇵🇫"
  },
  {
    "code": "PTP",
    "city": "Guadeloupe",
    "name": "Pointe-à-Pitre",
    "country": "Guadeloupe",
    "flag": "🇬🇵"
  },
  {
    "code": "FDF",
    "city": "Fort-de-France",
    "name": "Martinique Aimé Césaire",
    "country": "Martinique",
    "flag": "🇲🇶"
  },
  {
    "code": "SXM",
    "city": "Saint-Martin",
    "name": "Princess Juliana",
    "country": "Saint-Martin",
    "flag": "🇸🇽"
  },
  {
    "code": "NCY",
    "city": "Annecy",
    "name": "Annecy Haute-Savoie",
    "country": "France",
    "flag": "🇫🇷"
  },
  {
    "code": "VIE",
    "city": "Vienne",
    "name": "Wien-Schwechat",
    "country": "Autriche",
    "flag": "🇦🇹"
  },
  {
    "code": "ZRH",
    "city": "Zurich",
    "name": "Zürich",
    "country": "Suisse",
    "flag": "🇨🇭"
  },
  {
    "code": "CPH",
    "city": "Copenhague",
    "name": "Kastrup",
    "country": "Danemark",
    "flag": "🇩🇰"
  },
  {
    "code": "OSL",
    "city": "Oslo",
    "name": "Gardermoen",
    "country": "Norvège",
    "flag": "🇳🇴"
  },
  {
    "code": "ARN",
    "city": "Stockholm",
    "name": "Arlanda",
    "country": "Suède",
    "flag": "🇸🇪"
  },
  {
    "code": "HEL",
    "city": "Helsinki",
    "name": "Helsinki-Vantaa",
    "country": "Finlande",
    "flag": "🇫🇮"
  },
  {
    "code": "WAW",
    "city": "Varsovie",
    "name": "Chopin",
    "country": "Pologne",
    "flag": "🇵🇱"
  },
  {
    "code": "PRG",
    "city": "Prague",
    "name": "Václav Havel",
    "country": "Tchéquie",
    "flag": "🇨🇿"
  },
  {
    "code": "BUD",
    "city": "Budapest",
    "name": "Ferenc Liszt",
    "country": "Hongrie",
    "flag": "🇭🇺"
  },
  {
    "code": "OTP",
    "city": "Bucarest",
    "name": "Henri Coandă",
    "country": "Roumanie",
    "flag": "🇷🇴"
  },
  {
    "code": "SOF",
    "city": "Sofia",
    "name": "Sofia",
    "country": "Bulgarie",
    "flag": "🇧🇬"
  },
  {
    "code": "SVO",
    "city": "Moscou",
    "name": "Sheremetyevo",
    "country": "Russie",
    "flag": "🇷🇺"
  },
  {
    "code": "LED",
    "city": "Saint-Pétersbourg",
    "name": "Pulkovo",
    "country": "Russie",
    "flag": "🇷🇺"
  },
  {
    "code": "GYD",
    "city": "Bakou",
    "name": "Heydar Aliyev",
    "country": "Azerbaïdjan",
    "flag": "🇦🇿"
  },
  {
    "code": "TBS",
    "city": "Tbilissi",
    "name": "Shota Rustaveli",
    "country": "Géorgie",
    "flag": "🇬🇪"
  },
  {
    "code": "EVN",
    "city": "Erevan",
    "name": "Zvartnots",
    "country": "Arménie",
    "flag": "🇦🇲"
  },
  {
    "code": "NQZ",
    "city": "Astana",
    "name": "Nursultan Nazarbayev",
    "country": "Kazakhstan",
    "flag": "🇰🇿"
  },
  {
    "code": "TAS",
    "city": "Tachkent",
    "name": "Tachkent",
    "country": "Ouzbékistan",
    "flag": "🇺🇿"
  }
];

const byCode=new Map(airports.map((airport)=>[airport.code,airport]));
const popularCodes=["CDG","DXB","JFK","LHR","AMS","MAD","BCN","FCO","IST","CMN","ABJ","FIH"];
export const popularAirports=popularCodes.map((code)=>byCode.get(code)).filter((airport):airport is Airport=>Boolean(airport));
const normalize=(value:string)=>value.normalize("NFD").replace(/[\u0300-\u036f]/g,"").toLowerCase();
export function searchAirports(query:string){const q=normalize(query.trim());if(!q)return popularAirports;return airports.filter((airport)=>[airport.code,airport.city,airport.country,airport.name].some((value)=>normalize(value).includes(q))).sort((a,b)=>{const exactA=normalize(a.code)===q?0:normalize(a.code).startsWith(q)?1:normalize(a.city).startsWith(q)?2:3;const exactB=normalize(b.code)===q?0:normalize(b.code).startsWith(q)?1:normalize(b.city).startsWith(q)?2:3;return exactA-exactB||a.city.localeCompare(b.city);}).slice(0,30);}
