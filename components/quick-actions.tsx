...
export function QuickActions() {
  ...
  return <><section className="quick-actions-section"><div className="section-heading"><h2>Actions rapides</h2><span>Sans quitter l’accueil</span></div><div className="quick-actions-scroll">{actions.map((item) => <button className={`quick-action quick-${item.tone}`} onClick={() => reset(item.key)} key={item.key}><span><item.icon size={17} /></span>{item.label}</button>)}</div></section>
    ...
    <Sheet open={action === "subscription"} title="Ajouter un abonnement" submitLabel="Ajouter" disableSubmit={!title.trim() || amount <= 0} onClose={() => setAction(undefined)} onSubmit={saveSubscription} icon={RefreshCcw} tone="rose"><div className="transaction-form"><Field label="Montant mensuel"><AmountField value={amount} onChange={setAmount} autoFocus /></Field><FormSection title="Détails"><Field label="Nom"><input className="input" value={title} onChange={(event) => setTitle(event.target.value)} /></Field><Field label="Catégorie"><input className="input" value={category} onChange={(event) => setCategory(event.target.value)} /></Field><Field label="Jour de prélèvement"><input className="input" type="number" min="1" max="31" value={dueDay} onChange={(event) => setDueDay(Number(event.target.value))} /></Field></FormSection></div></Sheet>
  </>;
}
