...
export function QuickActions() {
  ...
  return <><section className="quick-actions-section"><div className="section-heading"><h2>Actions rapides</h2><span>Sans quitter l’accueil</span></div><div className="quick-actions-scroll">{actions.map((item) => <button className={`quick-action quick-${item.tone}`} onClick={() => reset(item.key)} key={item.key}><span><item.icon size={17} /></span>{item.label}</button>)}</div></section>
    <FormModal open={action === "expense" || action === "income"} title={budgetType === "revenu" ? "Ajouter un revenu" : "Ajouter une dépense"} submitLabel="Enregistrer la transaction" disableSubmit={!title.trim() || amount <= 0} onClose={() => setAction(undefined)} onSubmit={() => saveBudget(budgetType)} icon={budgetType === "revenu" ? TrendingUp : CircleDollarSign} tone={budgetType === "revenu" ? "green" : "red"}><div className="form-grid"><FormSection title="Type"><AnimatedSegmented value={budgetType} options={[{ value: "depense", label: "↓ Dépense" }, { value: "revenu", label: "↑ Revenu" }]} onChange={(type) => setAction(type === "revenu" ? "income" : "expense")} label="Type" /></FormSection><FormSection title="Transaction"><FormRow><Field label="Montant"><AmountField size="modal" value={amount} onChange={setAmount} autoFocus /></Field><Field label="Catégorie"><input className="input" value={category} onChange={(event) => setCategory(event.target.value)} /></Field></FormRow><div className="form-choice-chips" aria-label="Catégories rapides">{["Logement", "Alimentation", "Transport", "Voyage", "Abonnement"].map((item) => <button type="button" className={category === item ? "active" : ""} aria-pressed={category === item} onClick={() => setCategory(item)} key={item}>{item}</button>)}</div><Field label="Intitulé"><input className="input" value={title} autoCapitalize="sentences" onChange={(event) => setTitle(event.target.value)} /></Field><Field label="Date"><DateField value={toDateInput(date)} onChange={(v) => setDate(fromDateInput(v))} /></Field></FormSection></div></FormModal>
    ...
    <FormModal open={action === "subscription"} title="Ajouter un abonnement" submitLabel="Ajouter l’abonnement" disableSubmit={!title.trim() || amount <= 0} onClose={() => setAction(undefined)} onSubmit={saveSubscription} icon={RefreshCcw} tone="rose"><div className="form-grid"><FormSection title="Abonnement"><Field label="Montant mensuel"><AmountField size="modal" value={amount} onChange={setAmount} autoFocus /></Field><Field label="Nom"><input className="input" value={title} onChange={(event) => setTitle(event.target.value)} /></Field><FormRow><Field label="Catégorie"><input className="input" value={category} onChange={(event) => setCategory(event.target.value)} /></Field><Field label="Jour de prélèvement"><input className="input" type="number" min="1" max="31" value={dueDay} onChange={(event) => setDueDay(Number(event.target.value))} /></Field></FormRow></FormSection></div></FormModal>
  </>;
}
