...
import { airports } from "@/lib/airports/airports";
...
type Kind = "flight" | "stay" | "activity";
type Draft = {
...
const blank = (date: string): Draft => ({
  ...
  durationMinutes: 120, pricePerPerson: false, category: "visite",
});
...
export function TripItineraryPanel({ trip }: { trip: Trip }) {
  ...
  const editStay = (item: Accommodation) => { setKind("stay"); setEditing(item.id); setDraft({ ...blank(item.startDate), title: item.name, city: item.city, startDate: item.startDate, endDate: item.endDate, price: item.price, status: item.status, checkInTime: item.checkInTime ?? "15:00", checkOutTime: item.checkOutTime ?? "11:00" }); };
  ...
  const save = () => {
    ...
    if (kind === "flight") {
      ...
      const payload = { tripId: trip.id, name: draft.title.trim(), city: draft.city.trim(), startDate: draft.startDate, endDate: draft.endDate, price: draft.price, bookingLink: "", attachmentNote: "", status: draft.status, checkInTime: draft.checkInTime || undefined, checkOutTime: draft.checkOutTime || undefined, imageUrl: "" };
