...
export default function RentalsPage() {
  ...
  return <>
    <main className="page v2-page v2 rentals-page">
      ...
      <Sheet open={tenantOpen} title={editing ? "Modifier le locataire" : "Ajouter un locataire"} submitLabel={editing ? "Enregistrer" : "Ajouter"} disableSubmit={!tenantDraft.name.trim() || tenantDraft.monthlyRent <= 0} onClose={() => setTenantOpen(false)} onSubmit={saveTenant}><div className="form-grid"><Field label="Nom"><input className="input" value={tenantDraft.name} onChange={(event) => setTenantDraft({ ...tenantDraft, name: event.target.value })} /></Field><Field label="Loyer mensuel"><input className="input" type="number" inputMode="decimal" value={tenantDraft.monthlyRent || ""} onChange={(event) => setTenantDraft({ ...tenantDraft, monthlyRent: Number(event.target.value) })} /></Field><Field label="Jour d'échéance"><input className="input" type="number" min="1" max="31" value={tenantDraft.dueDay} onChange={(event) => setTenantDraft({ ...tenantDraft, dueDay: Number(event.target.value) })} /></Field><Field label="Note"><textarea className="textarea" value={tenantDraft.note} onChange={(event) => setTenantDraft({ ...tenantDraft, note: event.target.value })} /></Field></div></Sheet>
