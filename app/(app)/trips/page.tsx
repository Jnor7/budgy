...
import { CountrySelector } from "@/components/travel/country-selector";
...
export default function TripsPage() {
  ...
  const resolveCover = async (trip: Trip, country: string, excludePhotoId = "") => {
    const image = await destinationImageProvider.findLandscape(country, "", { tripId: trip.id, excludePhotoId });
  ...
  const refreshEditingCover = async () => {
    ...
    const refreshed = await resolveCover({ ...editing, ...destination }, destination.countryName, editing.coverImageId);
    showToast(refreshed
  ...
  const save = () => {
    ...
    if (editing) {
      ...
      if (!editing.coverImageUrl) void resolveCover({ ...editing, ...payload }, countryName);
      ...
      void resolveCover(trip, countryName);
