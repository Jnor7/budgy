...
const logPipeline = (values: Diagnostic & { tripId: string }) => {
  const event = {
    provider: "unsplash",
    query: values.query,
...
export async function GET(request: Request) {
  ...
  const tripId = searchParams.get("tripId")?.trim().slice(0, 80) ?? "";
  ...
  try {
    ...
    if (photo.links?.download_location) {
      ...
      if (!download?.ok) logPipeline({ query, status: download?.status ?? 0, errorType: "download_tracking_failed", resultsCount: results.length, tripId });
    }
    logPipeline({ query, status: response.status, errorType: "", resultsCount: results.length, tripId });
