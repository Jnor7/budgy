...
export async function GET(request: Request) {
  ...
  if (!accessKey || !destination) return Response.json(fallback(), { headers: { "Cache-Control": "no-store" } });
  ...
  try {
    ...
    if (!photo || !raw) return Response.json(fallback(), { headers: { "Cache-Control": "no-store" } });
